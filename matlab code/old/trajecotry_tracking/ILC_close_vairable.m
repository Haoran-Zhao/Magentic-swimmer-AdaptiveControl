...
%     control_ILC = [control_hist(ind,1,1);control_hist(ind,1,2);control_hist(ind,1,3)] + learningRate.*(lead_pt' - X(1:3,i));
    control_ILC = control_last + learningRate1.*norm(lead_pt' - X(1:3,i))+learningRate2.*norm(cp - X(1:3,i));
%     orient_ILC = orient_last+ learningRate1.*(lead_pt' - X(1:3,i))/norm((lead_pt' - X(1:3,i)))+learningRate2.*(cp - X(1:3,i))/norm((cp - X(1:3,i)));
    orient_ILC = orient_last+ learningRate1.*(lead_pt' - X(1:3,i))/norm((lead_pt' - X(1:3,i)))+learningRate2.*(cp - X(1:3,i))/norm((cp - X(1:3,i)));

    %orient_ILC = orient_ILC/norm(orient_ILC);
    control_cl = Kp*(lead_pt' - X(1:3,i)) + Ki*errInt - Kd*vel;
    if (norm(control_ILC)+norm(control_cl))>thrust
        control = thrust * ((orient_ILC+control_cl)/norm(control_cl+orient_ILC));
    else
        control = (norm(control_ILC)+norm(control_cl))*((orient_ILC+control_cl)/norm(control_cl+orient_ILC)); % PID type ILC
    end
    %control_hist(ind,1,:) = control ; 
    % controller steers the thrust (only controls the orientation)
    u = control;
    disp(u)
    control_last = norm(control);
    orient_last = ((orient_ILC+control_cl)/norm(control_cl+orient_ILC));
